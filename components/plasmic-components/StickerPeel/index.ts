...
export const StickerPeelMeta = {
  ...
  props: {
    ...
    fillColor:      { type: 'color',   defaultValueHint: '#ffffff',   description: 'Fill of the sticker body. Set the color HERE rather than on the slot content — the peeled corner is a real hole in this shape, so whatever is behind the sticker shows through it.' },
    borderRadius:   { type: 'number',  defaultValueHint: 0,           description: 'Corner radius in px for the three un-peeled corners. Set this here rather than styling border-radius on the instance, which would square off the peel notch. The peeled corner stays a sharp diagonal fold.' },
